...
def omg_llava_collate_fn(instances: Sequence[Dict],
    ...
    has_region = any(inst.get('regions') is not None for inst in instances)
    ...
    for example in instances:
        ...
        if has_mask:
            ...
            # object_masks.append(example['masks'] if 'masks' in example.keys() else None)
        if has_region:
            if 'regions' in example.keys() and example['regions'] is not None:
                object_regions.append(example['regions'])
        if has_points:
    ...
    if has_mask:
        if len(object_masks) == 0:
            object_masks = torch.zeros((0, pixel_values.shape[-2], pixel_values.shape[-1]))
        else:
    ...
    if has_region:
        if len(object_regions) == 0:
            object_regions = torch.zeros((0, pixel_values.shape[-2], pixel_values.shape[-1]))
        else:
            object_regions = torch.cat(object_regions, dim=0)
        data_dict['regions'] = object_regions
    if has_points:
        if len(prompt_points) == 0:
            prompt_points = torch.zeros((0, 2))
        else:
            prompt_points = torch.cat(prompt_points, dim=0)
