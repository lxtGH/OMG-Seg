...
class OpenCLIPBackbone(BaseModule):
    ...
    def __init__(
        ...
        self.config = Data
        # self.config.hidden_size = output_channels[-2]
    ...
    def forward_feat(self, features):
        if self.model_type == 'convnext':
            ...
            return (self.head(features) @ self.proj)[:, 0]  # should return n x c
    ...
    def forward(self, x, output_hidden_states=True):
        ...
        # print([item.shape for item in outs])
        # outs = outs[-2].flatten(2).permute(0, 2, 1)
